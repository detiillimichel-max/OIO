...
export function QuantumDots({ activeIndex, onNavigate, total = 3 }: QuantumDotsProps) {
  ...
  return (
    <nav 
      className="fixed right-6 top-1/2 -translate-y-1/2 z-30 flex flex-col gap-5"
      aria-label="Navegação OIO"
    >
        ...
        >
          {ripples
            ...
            .map((ripple) => (
              <span
                key={ripple.id}
                className="absolute inset-0 m-auto w-2 h-2 rounded-full bg-emerald-500/30 animate-ripple pointer-events-none"
              />
            ))}
          ...
          <span
            className={cn(
              ...
              activeIndex === index
                ...
                : 'bg-zinc-700 group-hover:bg-zinc-500 group-hover:scale-125'
            )}
          />
        </button>
      ))}
